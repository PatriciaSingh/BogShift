---
title: "Tutorial 1: Retrieving Species Data from GBIF"
author: "Your Name"
date: "`r Sys.Date()`"
output: 
  html_document:
...
````
````{r configure-environment-settings, include=FALSE}
...
> **🔧 Technical Note**: The configuration above ensures your tutorial works both 
> interactively and when knitting, with appropriate timeout settings for large downloads.
...
````{r total-gbif-observations}
occ_count(basisOfRecord='OBSERVATION')
````

> **💡 Tip**: For Species Distribution Modeling, observations are often preferred 
...
````{r help-occ-search-parameters, eval=FALSE}
?occ_search
````
...
````{r download-gbif-data, eval=FALSE, cache=TRUE}
...
````{r import-downloaded-data}
# Use cached file if available, otherwise use fresh download
